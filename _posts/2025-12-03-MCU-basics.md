...
* **이 기능들을 실제로 제공하는 주변장치(하드웨어)를**를 통칭하여 패리펄럴이라고하며 **MCU 내부에** 다양한 패리펄럴이 내장되어있음  
...
### 레지스터(Register) 개념
* MCU 주요 메모리 장치 (Flash, RAM, Peripheral의 Register)   
cf. CPU 내부에도 레지스터가 있지만 성격이 다름 - CPU 속의 레지스터를 접근하기 위해 메모리 주소가 필요없음
...
→ MCU SW(CPU) 기능 : 메모리에서 값을 읽어온다 → 읽어온 값으로 사칙연산, 조건문 판단, 반복문 수행... → 메모리에 값을 쓴다
* 각 **패리펄럴 내부**에는 여러 개의 레지스터(**작은 크기의 메모리**)가 존재
...
* **Driver SW (HAL SW)** 
    * **API 형태로 특정 패리펄럴 레지스터에 값을 읽고 쓰도록하는 소프트웨어를 미리 구현**하여 사용자 구현된 API를 호출하여 사용하여 가독성, 재사용성 향상  
    → IDE에서 매크로 및 심볼이 정의된 헤더 파일(.h)을 include하여 사용자가 일일이 레지스터 읽고 쓰기 구현 불필요, 빌드 과정에서 대응되는 실제 메모리 주소로 치환  
...
* **어셈블리어 / 기계어 개념**
    ...
    * CPU마다 레지스터의 수와 크기가 다르며 보통 8bit / 16bit / 32bit 형태로 구성됨. (ex. MIPS는 32bit 레지스터 32개 보유)
    ...
    * 예시 ($표시 있으면 레지스터를 의미)
    ...
    <p align="center"><b>표 1. C언어 · 어셈블리어 · 기계어(16진수) 비교 예시 (MIPS) </b></p>
    ...
    |add  $a $b $c|$a = $b + $c|
    |:|:|
    <p align="center"><b>표 2. 기계어 명령어 (MIPS) </b></p>
...
* **HEX / ELF 파일 개념** 
    ...
    * 생성된 기계어가 Flash 메모리의 어느 주소에 다운로드될지 결정 필요
    → **Hex 파일에는 기계어와 해당 기계어가 기록될 메모리 주소 정보가 함께 포함** 
    * Hex 파일 속 주소값은 컴파일러가 빌드 과정에서 자동으로 할당하며, 이는 사용 MCU의 메모리 맵을 기준으로 결정 
    → 빌드 과정(Compile → Assemble → Link) 중, 메모리 주소 할당은 링크(Link) 단계에서 수행하며 이때 **링커 스크립트(Linker Script)** 를 참조하여 메모리 주소 할당  
    ...
    * ELF 파일을 MCU Flash에 다운로드할 때도 실제 MCU에 전달되 것은 기계어 코드만이며, 디버깅 정보는 MCU가 아닌 디버거로 전달  
    → SW를 개발하는 과정중에는 디버깅을 위해서 ELF 파일을 사용하는것이 일반적이지만, 개발이 완료된 후 실제 제품에 SW가 다운로드될때는 HEX파일만 사용
