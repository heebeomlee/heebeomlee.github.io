...
→ MCU의 전원을 키면 바로 Main 함수가 실행되는게 아니라 초기화(전역변수 추기화 등) 작업을 거치게됨. hex 파일안에는 메인 함수에 대한 기계어가 들어있을 뿐 아니라 초기화 작업을 해주는 Startup Code도 hex 파일 앞부분에 위치. IDE가 설치된 폴더 안에 별도의 Startup 코드가 존재하여(.o, .s 등) 코드 빌드시 메인함수와 빌드되어 생성 (협업에서 default를 사용하지 않고 사용자가 startup 코드를 별도로 만들어야하는 경우도 있음, startup 코드 이후 main함수를 불러옴)  
...
## 메모리 개념
* 메모리 : 값을 저장할 수 있는 기억장치이며 크게 2가지 Type로 분류
1. ROM : 전원 Off해도 지워지지 안음 (하드디스크, SSD, **Flash** 등)  
값을 쓰는 절차가 RAM에 비해 복잡함  (중간에 Flash Controller가 있어 레지스터에 적절한 값을 적절한 순서로 써야 Flash에 값을 쓰고 읽음)
값을 읽는 속도가 RAM에 비해 오래걸림
2. RAM : 전원 Off하면 값이 초기화 (SRAM, DRAM, 등)  
값을 쓰는 절차가 간편함 (기계어 명령어 한줄 가능 ex. 기계어 명령 형태 : RAM에 값 5를 써라, RAM에서 값을 읽어와라 등)  
값을 읽는 속도가 ROM보다 빠르다
* 메모리 용량 얼마나 많은 데이터를 저장할 수 있는지 나타내는 지표 (단위 : Byte)  
→ 1024Byte = 1KB, 1024KB = 1MB, 1024MB = 1GB, 1024GB = 1TB
* 메모리 주소 : 메모리에 어디에 접근으르 해서 데이터를 쓰고/읽을지 정의 필요  
→ 특정 메모리 위치를 표현  
→ 메모리는 바이트 단위로 **고유한** 주소 할당 (MCU제조사에서 할당 후 Data Sheet에 명시)  
→ 정의되지 않은 (할당되지 않은) 메모리는 Reserved라고 함 (CPU가 Reserved Memory에 접근 시 Error 또는 비정상 동작 가능, C언어 Pointer로 메모리 주소에 접근시 주소 오류, Complier 설정 잘못할시 발생 가능)
→ 하나의 MCU에는 여러 종류의 **메모리** 가 있음 (Flash0, Flash1, RAM0, RAM1, RAM2, Peripheral Register1,Peripheral Register2 등) → 메모리 종류에 따른 고유 주소 할당 필요 (정보가 **Memory Map** 에 명시) (대부분의 MCU에서 메모리 주소를 16진수 hexa로 표현 = 0x, h 등)