...
> ### 메모리 개념
---
* **메모리** : 값을 저장할 수 있는 기억장치로 크게 2가지 종류로 분류
1. **ROM**  
    * 하드디스크, SSD, **Flash** 등 존재
    * 전원 Off해도 지워지지 않음
...
2. **RAM**  
    ...
    * 전원 Off하면 값이 초기화
    * 값을 쓰는 절차가 간편함  
    → 기계어 명령어 한줄 가능 (ex. 기계어 명령 형태 : RAM에 값 5를 써라, RAM에서 값을 읽어와라 등)     
    * 값을 읽는 속도가 ROM보다 빠르다  
* **메모리 용량**  
→ 얼마나 많은 데이터를 저장할 수 있는지 나타내는 지표 (단위 : Byte)  
→ 1024Byte = 1KB, 1024KB = 1MB, 1024MB = 1GB, 1024GB = 1TB
* **메모리 주소**   
→ MCU에서는 어떤 **메모리 주소(위치)**에 접근하여 데이터를 읽고/쓰는지 정의 필요  
