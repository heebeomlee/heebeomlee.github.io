...
* E2E 프로토콜의 핵심 요소
    - **CRC(Cyclic Redundancy Check)**: 메시지가 전송되는 동안 메시지의 내용이 변경되지 않았는지 검증
    - **Counter(Message Counter, Sequence Counter, Alive Counter)**: 카운터를 사용해 메시지 순서나 반복을 감지
* E2E 프로토콜 적용시 송신 메시지의 **데이터 영역**에 CRC와 Counter 값을 추가하여 메시지를 송신  
→ 세부 바이트 위치는 OEM이 정의하지만 일반적으로는 **첫 번째 바이트에 CRC**, **다음 바이트에 카운터** 방식으로 배치
* CAN 통신에서 자체적으로 사용하는 CRC와 E2E 프로토콜에서 사용하는 CRC는 다름
    - CAN의 CRC는 CAN Controller에서 자동으로 처리되며, 사용자는 별도로 CRC 계산 기능을 구현할 필요가 없는 반면 E2E 프로토콜은 소프트웨어적으로 CRC 값을 계산하여 메세지의 **데이터 영역**에 포함시켜야 합
<br>
<div style="text-align:center;">
...
1. **Counter**
    ...
    - **카운터를 통한 오류 감지**
        ...
        - **메시지 전달 순서 오류**: 카운터 값이 1, 2, 3, 4 순서대로 들어와야 하는데, 예를 들어 1, 3, 2, 4와 같이 순서가 바뀌면, 수신측은 이 순서 오류를 감지할 수 있음

2. **CRC**
    - **데이터 송신 과정에서 값이 변형되었는지 감지**하기 위한 통신 기법  
    - **CRC**와 **체크섬** 모두 데이터 오류를 탐지하는 방식이지만, 체크섬은 단순히 데이터 값을 합산한 결과를 함께 전송하여 수신 측에서 다시 데이터를 합산하여 검증하는 방식인 반면, CRC는 다항식 기반의 더 복잡한 연산을 통해 오류를 보다 정교하게 검출. 이떄, 원본 데이터 뿐 아니라 Counter 값도 CRC 계산에 활용
    - **CRC 계산 방법**
        - CRC 계산은 원본 데이터 뒤에 0을 추가한 후, XOR 나눗셈 연산을 수행하여 몫과 나머지를 구하며, 이때 나머지 값이 CRC가 됨
        - **Polynomial(다항식)** 은 CRC 계산 시 XOR 나눗셈에 사용되는 나누는 값이며, 어떤 폴리노미얼을 사용하느냐에 따라 CRC 계산값이 달라짐
        - CRC 계산에 사용되는 폴리노미얼은 **CRC 데이터 사이즈**등에 따라여러 종류가 존재하며, E2E 프로토콜 프로파일에 따라 사용할 폴리노미얼 값이 정의되어있음. 프로파일에 따라 계산된 CRC의 바이트 순서를 뒤집어서 삽입하는 경우도 있음
        - 동일한 폴리노미얼이라도 표현하는 방식에는 여러 가지가 존재
<br>
<div style="text-align:center;">
...
* **CRC - 데이터 ID**:
    - 인증 되지 않은 송신자가 메세지 송신 (Masquerading) 오류를 감지하기 위해 사용
    - 데이터 ID는 메시지에 직접 포함되지 않고 CRC 계산에만 사용되며 CRC 계산 시, 데이터 ID와 카운터를 결합하여 최종적인 CRC 값을 도출
    - 데이터 ID는 OEM이 네트워크 설계 시 미리 정하여 제어기 개발자에게 전달
    - ex. OEM : ID가 0x100인 CAN Message의 Data ID는 0x12, 0x34
    - ex. 프로파일 1에서는 데이터 ID가 2바이트로 정의되어 있으며, CRC를 계산할 때 이 값을 포함하여 계산. 이때 데이터 ID는 바이트 순서가 반대로 적용 (데이터 ID가 0x1234이면 CRC 계산 시 0x3412로 처리)
<br>
<div style="text-align:center;">
    <img src="../images/2025-12-04-CAN-Basics-Part2/2025-12-10-15-14-05.png" style="width:55%;" />
</div>
<p align="center"><strong>Data ID 활용 CRC 계산방법</strong></p>

* **E2E 프로토콜 Profile**
    - E2E 프로토콜에는 여러 가지 **Profile(프로파일)** 이 존재하며, 각 프로파일은 CRC, Counter를 계산하는 고유한 방식을 가지고 있음 (E2E Protocol Specification 문서 참조)  
    → ex. 각 프로파일은 카운터 계산시 **카운터 비트 크기**, **초기 카운터 값**, **카운터 값의 순환 방식**을 다르게 설정할 수 있음  
    → ex. 각 프로파일은 CRC 계산시 **Polynomial**, **Initial Value**, **XOR Value**을 다르게 설정할 수 있음
